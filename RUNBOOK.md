...
4. **Injection detection is pattern-based**, so it both misses rewordings and raises false positives. Measured on a real CV: a security researcher's CV was flagged for the phrase "system prompt" because their work involves auditing LLM system prompts. **The vocabulary of the field overlaps with the vocabulary of the attack**, so CVs from AI and security candidates will flag more often. It only ever raises a flag for a human; it never changes a verdict.
